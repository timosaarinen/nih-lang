...
export type ComptimeType = 'i32' | 'u32' | 'f32' | 'bool'
...
export interface ComptimeParam { name: string; type: ComptimeType }
export interface Param { name: string; type: TypeName }
...
export type Stmt =
  ...
  | { kind: 'comptime-if'; condition: Expr; thenBody: Stmt[]; elseBody: Stmt[]; line: number }
...
export type Expr =
  ...
  | { kind: 'call'; callee: string; comptimeArgs: Expr[]; args: Expr[]; line: number }
